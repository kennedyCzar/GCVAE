...
class gcvae_v1(keras.Model):
    def __init__(self, input_shape:tuple, 
        ...
        **kwargs : dict
        ...
        x = tf.keras.layers.Conv1D(64, 3, activation = tf.nn.relu, padding = 'same')(encoder_inputs) #Conv 1
        ...
        x = tf.keras.layers.Conv1DTranspose(32, 3, activation = tf.nn.relu, padding="valid")(x) #Deconv layer 1
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.UpSampling1D(2)(x)
        x = tf.keras.layers.Conv1DTranspose(64, 3, activation = tf.nn.relu, padding="same")(x) #Deconv layer 2
        if self.dist == 'b':
            decoder_outputs = layers.Conv1DTranspose(1,1, activation = tf.nn.sigmoid, padding="same")(x) #output layer
        else:
            decoder_outputs = layers.Conv1DTranspose(1,1, activation = tf.nn.relu, padding="same")(x) #output layer
        #---econder object --------------------------------------------------------------------------------------
