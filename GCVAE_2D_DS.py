...
class gcvae_v1(keras.Model):
    def __init__(self, input_shape:tuple, 
        ...
        **kwargs : dict
        ...
        x = tf.keras.layers.Conv2D(64, 3, activation = tf.nn.relu, padding = 'same')(encoder_inputs) #Conv 1
        x = tf.keras.layers.MaxPooling2D(2)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Conv2D(32, 3, activation = tf.nn.relu, padding = 'valid')(x) #Conv 2
        x = tf.keras.layers.MaxPooling2D(2)(x) 
        x = tf.keras.layers.BatchNormalization()(x)
        ...
        x = tf.keras.layers.Conv2DTranspose(32, 3, activation = tf.nn.relu, padding="valid")(x) #Deconv layer 1
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.UpSampling2D(2)(x)
        x = tf.keras.layers.Conv2DTranspose(64, 3, activation = tf.nn.relu, padding="same")(x) #Deconv layer 2
        if self.dist == 'b':
            decoder_outputs = layers.Conv2DTranspose(1,1, activation = tf.nn.sigmoid, padding="same")(x) #output layer
        else:
            decoder_outputs = layers.Conv2DTranspose(1,1, activation = tf.nn.relu, padding="same")(x) #output layer
        #---deconder object --------------------------------------------------------------------------------------
    ...
    def vae_univ_gauss(mu, lg_sigma, r_loss, beta = None, vloss = 'elbo'):
        ...
        if vloss == 'elbo':
            ...
            beta = PIDControl_v2().pid(16, kl_loss) #weight on KL-divergence
        else:
...
class gcvae_v2(keras.Model):
    def __init__(self, input_shape:tuple, 
        ...
        **kwargs : dict
        ...
        x = tf.keras.layers.MaxPooling2D(2)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Conv2D(64, 3, activation = tf.nn.relu, padding = 'same')(x) #Conv 2
        x = tf.keras.layers.MaxPooling2D(2)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Conv2D(32, 3, activation = tf.nn.relu, padding = 'same')(x) #Conv 3
        x = tf.keras.layers.MaxPooling2D(3)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Conv2D(32, 3, activation = tf.nn.relu, padding = 'same')(x) #Conv 4
        x = tf.keras.layers.MaxPooling2D(2)(x) 
        x = tf.keras.layers.BatchNormalization()(x)
        max_pool_sz = x.shape[1:] #extract shape of final convolution
        x = layers.Flatten()(x)
        ...
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.UpSampling2D(2)(x)
        x = tf.keras.layers.Conv2DTranspose(32, 3, activation = tf.nn.relu, padding="valid")(x) #Deconv layer 2
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.UpSampling2D(2)(x)
        x = tf.keras.layers.Conv2DTranspose(64, 3, activation = tf.nn.relu, padding="valid")(x) #Deconv layer 3
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.UpSampling2D(2)(x)
        x = tf.keras.layers.Conv2DTranspose(64, 3, activation = tf.nn.relu, padding="valid")(x) #Deconv layer 4
        if self.dist == 'b':
            decoder_outputs = layers.Conv2DTranspose(1, 3, activation = tf.nn.sigmoid, padding="valid")(x) #output layer
        else:
            decoder_outputs = layers.Conv2DTranspose(1, 3, activation = tf.nn.relu, padding="valid")(x) #output layer
        #---deconder object --------------------------------------------------------------------------------------
        decoder = keras.Model(latent_inputs, decoder_outputs, name = "decoder")
        self.decoder = decoder
        #--- loss function tracker
